...
class GTAW_Control
{
    public:
        ...
        int HandleConfig(const vector<string> &param); //手柄开关设置
        int CleanError(); //麦格米特清错
        int ChangeMode(const vector<string> &param); //修改麦格米特模式
    public:
    ...
    private:
        ...
        string boxIO;
        int m_isArcOnFlag;
        int m_handleEnable =0; //手柄6个按钮开关，0：关；1：开
        atomic<int> m_megMode; //麦格米特模式
        // mutex m_oMutex;
	    ...
	    atomic_char HyperthermRS485Data[31];
	    atomic_char Temp_HyperthermRS485Data[31];
